...
export default function RegisterForm() {
  ...
  const onSubmit = async (data: RegisterPayload) => {
    try {
      const { token, user } = await registerMutation.mutateAsync(data);
  ...
  return (
    <div className="w-full max-w-md mx-auto py-12 px-6 bg-zinc-900 rounded-md">
      <div className="text-center mb-8">
        ...
        <p className="text-zinc-400 text-sm mt-1">Create an account to start selling or shopping</p>
      </div>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row justify-between text-sm text-zinc-400 mt-4">
            ...
            <NavLink to="/forgot-password" className="text-green-500 hover:underline mt-2 sm:mt-0">
              Forgot password?
            </NavLink>
          </div>
        </form>
